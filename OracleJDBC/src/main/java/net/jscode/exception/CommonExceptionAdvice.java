...
public class CommonExceptionAdvice {
	...
	// 컨트롤러에서 예외처리를 담당까지하면 많은 코드가 중복 발생이 됨
	// 예외처리용 코드를 분리하여 공통으로 관리한다.
	
	@ExceptionHandler(Exception.class) //예외처리용 코드 메서드
	public String except(Exception ex, Model model) {
		log.error("except 메서드 실행..."+ ex.getMessage()); //예외처리 메시지를 콘솔에 출력
	...
	@ResponseStatus(HttpStatus.NOT_FOUND) //404 상태코드 일때
